...
class PokemonRedReader:
    ...
    _PARTY_BASE_ADDRS = [0xD16B, 0xD197, 0xD1C3, 0xD1EF, 0xD21B, 0xD247]
    _PARTY_NICK_ADDRS = [0xD2B5, 0xD2C0, 0xD2CB, 0xD2D6, 0xD2E1, 0xD2EC]

    # Pre-calculated character mapping for performance
    _CHAR_MAP = {i: f"[{i:02X}]" for i in range(256)}
    ...
    def read_money(self) -> int:
        ...
        # Read 3 bytes at once for performance
        b3, b2, b1 = self.memory[0xD347:0xD34A]
        money = (
            ((b3 >> 4) * 100000)
            + ((b3 & 0xF) * 10000)
            + ((b2 >> 4) * 1000)
    ...
    def _convert_text(self, bytes_data: list[int]) -> str:
        """Convert Pokemon text format to ASCII using pre-calculated mapping"""
        result = []
        for b in bytes_data:
            if b == 0x50:  # End marker
                break
            result.append(self._CHAR_MAP[b])
    ...
    def read_party_pokemon(self) -> list[PokemonData]:
        ...
        for i in range(party_size):
            addr = self._PARTY_BASE_ADDRS[i]

            # Read experience (3 bytes) using slicing
            e1, e2, e3 = self.memory[addr + 0x1A : addr + 0x1D]
            exp = (e1 << 16) + (e2 << 8) + e3
            ...
            nickname = self._convert_text(
                self.memory[self._PARTY_NICK_ADDRS[i] : self._PARTY_NICK_ADDRS[i] + 11]
            )
            ...
            # Read HP and Trainer ID using slicing
            hp_cur_raw = self.memory[addr + 1 : addr + 3]
            hp_max_raw = self.memory[addr + 0x22 : addr + 0x24]
            tr_id_raw = self.memory[addr + 12 : addr + 14]

            pokemon = PokemonData(
                species_id=self.memory[addr],
                species_name=species_name,
                current_hp=(hp_cur_raw[0] << 8) + hp_cur_raw[1],
                max_hp=(hp_max_raw[0] << 8) + hp_max_raw[1],
                ...
                moves=moves,
                move_pp=move_pp,
                trainer_id=(tr_id_raw[0] << 8) + tr_id_raw[1],
                nickname=nickname,
                experience=exp,
            )
    ...
    def read_dialog(self) -> str:
        ...
        # Get all bytes from the buffer using slicing for performance
