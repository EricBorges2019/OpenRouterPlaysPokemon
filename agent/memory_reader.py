...
class PokemonRedReader:
    ...
    # Pre-calculated character map for faster text conversion
    _CHAR_MAP = {
        0x4E: "\n",
        0x7F: " ",
        0x6D: ":",
    ...
    # Initialize ranges (A-Z, a-z, 0-9)
    for i in range(0x1A):
        _CHAR_MAP[0x80 + i] = chr(ord("A") + i)
        _CHAR_MAP[0xA0 + i] = chr(ord("a") + i)
    for i in range(10):
        _CHAR_MAP[0xF6 + i] = str(i)
    ...
    def _convert_text(self, bytes_data: list[int]) -> str:
        """Convert Pokemon text format to ASCII using a fast lookup table"""
        result_parts = []
        for b in bytes_data:
    ...
    def read_pokedex_caught_count(self) -> int:
        ...
        for addr in range(0xD2F7, 0xD30A):
            ...
            # Count set bits in this byte using efficient bit_count (Python 3.10+)
